...
# El historial original se conserva; únicamente se adapta para aceptar
# una imagen procesada PIL generada por el flujo Gemini-only.
def guardar_analisis_en_google(uploaded_image, backend_result):
    ...
    if isinstance(annotated, Image.Image):
        ...
        try:
            ...
            raise RuntimeError(f"No se pudo convertir la imagen procesada a PNG: {exc}")

    nombre_procesada = f"{Path(uploaded_image.name).stem}_procesada.png"
    processed_file_id = subir_bytes_google_drive(
        processed_bytes,
        nombre_procesada,
        "image/png",
    ...
    backend_data = backend_result.get("backend") or {}
    analisis = backend_data.get("analisis", {}) if isinstance(backend_data, dict) else {}
    ...
    registro = {
        ...
        "nivel_visual": str(backend_result.get("nivel_afectacion_visual", "No determinado") or "No determinado"),
        "zona_mas_afectada": str(backend_result.get("zona_mas_afectada", "No determinada") or "No determinada"),
